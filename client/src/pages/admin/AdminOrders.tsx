        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedOrder && (
            <div className="space-y-6">
              ...
              {/* Adresse de livraison */}
              {((selectedOrder as any).deliveryStreet || (selectedOrder as any).deliveryCity) && (
                <div className="p-3 rounded-lg bg-blue-50/50 dark:bg-blue-950/20 border border-blue-200/50 dark:border-blue-800/30">
                  <p className="text-sm font-semibold text-blue-800 dark:text-blue-400 mb-1 flex items-center gap-1.5">
                  ...
                  <div className="text-sm space-y-0.5">
                    {(selectedOrder as any).deliveryContactName && (
                      <p className="font-medium">{(selectedOrder as any).deliveryContactName}</p>
                    )}
                    <p>{(selectedOrder as any).deliveryStreet}</p>
                    {(selectedOrder as any).deliveryStreet2 && <p>{(selectedOrder as any).deliveryStreet2}</p>}
                    <p>{(selectedOrder as any).deliveryPostalCode} {(selectedOrder as any).deliveryCity}</p>
                    {(selectedOrder as any).deliveryCountry && <p>{(selectedOrder as any).deliveryCountry}</p>}
                    {(selectedOrder as any).deliveryContactPhone && (
                      <p className="text-muted-foreground mt-1">Tél: {(selectedOrder as any).deliveryContactPhone}</p>
                    )}
                    {(selectedOrder as any).deliveryInstructions && (
                      <p className="text-muted-foreground mt-1 italic">Instructions: {(selectedOrder as any).deliveryInstructions}</p>
                    )}
                  </div>
                </div>
              )}

              {/* Adresse de facturation (depuis le partenaire) */}
              {selectedOrder.partner && (() => {
                const p = selectedOrder.partner as any;
                ...
                return (street || city) ? (
                  <div className="p-3 rounded-lg bg-purple-50/50 dark:bg-purple-950/20 border border-purple-200/50 dark:border-purple-800/30">
                    <p className="text-sm font-semibold text-purple-800 dark:text-purple-400 mb-1 flex items-center gap-1.5">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8Z"/><path d="M14 2v6h6"/><path d="M16 13H8"/><path d="M16 17H8"/><path d="M10 9H8"/></svg>
                    ...
                    <div className="text-sm space-y-0.5">
                      ...
                      {country && <p>{country}</p>}
                      {p.vatNumber && (
                        <p className="text-muted-foreground mt-1">TVA: {p.vatNumber}</p>
                      )}
                    </div>
                  </div>
                ) : null;
